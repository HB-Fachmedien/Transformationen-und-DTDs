		...
		<xsl:if test="not(/*/name()= ('divah', 'divsu', 'divso', 'gtdraft', 'vadraft', 'vav', 'entv', 'gts', 'gh' ))">
			<summary_plain>
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:value-of select="normalize-space(replace(substring(string-join(/*/body//p//text()[normalize-space()], concat(' ', codepoints-to-string(10))), 1, 500), '(\s\w*)$', ' '))"/>
					</xsl:otherwise>
				</xsl:choose>
			</summary_plain>
		</xsl:if>
						...
						<xsl:choose>
							...
							<xsl:when test="($pub-abbr = 'CF') or ($pub-abbr = 'CFL') or ($pub-abbr = 'CFB') or ($pub-abbr = 'FB')">
								...
								<node title="{$cf-title}" childOrder="BySequenceNr">
									...
									<xsl:choose>
										<xsl:when test="../name()='toc'"><!-- IHVs sollen zwei Ebenen höher einsortiert werden: -->
											<leaf sequenceNr="1"/>
										</xsl:when>
										<xsl:when test="../name()='ed'"><!-- IHVs sollen zwei Ebenen höher einsortiert werden: -->
											<leaf sequenceNr="5"/>
										</xsl:when>
										<xsl:when test="../name()='gk'"><!-- IHVs sollen zwei Ebenen höher einsortiert werden: -->
											<leaf sequenceNr="7"/>
										</xsl:when>
										<xsl:when test="descendant::pubedition = '00'">
											<leaf>
